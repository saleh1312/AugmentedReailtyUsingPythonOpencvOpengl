...
class draw3d:
    ...
    def draw_background(self, frame):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,
                     im.shape[0], im.shape[1], 0, GL_RGB, GL_UNSIGNED_BYTE, im)
    ...
    def __init__(self, focal_lengh, size):
        ...
        self.size = size
        self.video = cv2.VideoWriter('outpy.avi', cv2.VideoWriter_fourcc(
            'M', 'J', 'P', 'G'), 25, (size, size))
...
while True:
    ...
    if isinstance(data[0], type(None)):
        ...
        break
    elif isinstance(data[1], type(None)):
